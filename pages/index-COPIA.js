...
export default function Home({ initialPerfumes }) {
  ...
  const filtrados = useMemo(() => {
    const lista = perfumes.filter((p) => {
      ...
      const okTipo = tipo === "todos" || (p.tipo || "perfume") === tipo;

      return okCat && okTam && okPromo && okBusqueda && okTipo;
    });

    const ordenada = [...lista];

    ordenada.sort((a, b) => {
      // 1. Los disponibles siempre aparecen primero
      if (a.en_stock !== b.en_stock) {
        return a.en_stock ? -1 : 1;
      }

      // 2. Dentro de los disponibles o sin stock,
      // se aplica el orden seleccionado
      if (orden === "precio_asc") {
    ...
    return ordenada;
  }, [perfumes, categoria, tamano, soloPromos, busqueda, orden, tipo]);
      ...
      <div className="container">
        ...
        <div className="filters">
          <div className="filter-group">
            ...
            <button
              className={`pill ${tipo === "todos" ? "active" : ""}`}
              onClick={() => setTipo("todos")}
            >
              Todos
            </button>
            <button
              className={`pill ${tipo === "perfume" ? "active" : ""}`}
              onClick={() => setTipo("perfume")}
            >
              Perfumes
            </button>
            <button
              className={`pill ${tipo === "body_splash" ? "active" : ""}`}
              onClick={() => setTipo("body_splash")}
            >
              Body Splash
            </button>
          </div>
              ...
              <div className="card" key={p.id}>
                <div className="card-photo">
                  ...
                  <div className="card-badges-left">
                    ...
                    {p.tipo === "body_splash" && (
                      <span className="card-badge-tipo">💦 Body Splash</span>
                    )}
                    {p.en_promo && (
